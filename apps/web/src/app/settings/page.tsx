...
function SettingsInner() {
  ...
  return (
    <main className="mx-auto max-w-screen-sm pb-24">
      <PageHeader title="설정" back={false} />

      {/* 푸시 알림 */}
      <section className="px-4">
        <h2 className="mb-2 text-sm text-brand-700">푸시 알림</h2>
        <div className="rounded-xl border border-brand-100 bg-white p-4">
          <div className="flex items-center gap-3">
            <Icon name="notifications_active" className="text-brand-500" />
            <div className="flex-1">
              <p className="text-base text-brand-900">앱 푸시 알림</p>
            ...
            {pushState !== "granted" && pushState !== "unsupported" && (
              <button
                ...
                onClick={turnOnPush}
                className="inline-flex min-h-tap items-center rounded-lg bg-brand-500 px-3 py-2 text-sm text-white disabled:opacity-50"
              >
      ...
      <section className="mt-6 px-4">
        <h2 className="mb-2 text-sm text-brand-700">텔레그램 알림</h2>
        <div className="rounded-xl border border-brand-100 bg-white p-4">
          <div className="flex items-center gap-3">
            <Icon name="forum" className="text-brand-500" />
            <div className="flex-1">
              <p className="text-base text-brand-900">텔레그램 봇</p>
              <p className="text-sm text-brand-700">
            ...
            {tgLinked ? (
              <button
                ...
                className="inline-flex min-h-tap items-center rounded-lg border border-brand-200 px-3 py-2 text-sm text-brand-700"
              >
                해제
              </button>
            ) : (
          ...
          {tgCode && !tgLinked && (
            <div className="mt-3 rounded-lg bg-brand-50 p-3 text-sm text-brand-900">
              <p>
                텔레그램에서 <b>@{tgBotUsername || "개미청소봇"}</b>에 다음 메시지를
                보내세요:
              </p>
              <code className="mt-2 block rounded bg-white px-3 py-2 text-base font-semibold text-brand-700">
                /start {tgCode}
              </code>
              {tgExpiresAt && (
      ...
      {/* 계정 */}
      <section className="mt-6 px-4">
        <h2 className="mb-2 text-sm text-brand-700">계정</h2>
        <div className="rounded-xl border border-brand-100 bg-white p-4">
          <p className="text-sm text-brand-700">{user?.email || user?.phoneNumber}</p>
          <button
            type="button"
            onClick={signOut}
            className="mt-3 inline-flex min-h-tap items-center gap-1 rounded-lg border border-brand-200 px-3 py-2 text-sm text-brand-700"
          >
